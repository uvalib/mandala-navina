...
| [d7-user-role-migration-wipes-committed-role-permissions.md](d7-user-role-migration-wipes-committed-role-permissions.md) | migration / users / roles / config | Local smoke-test 2026-07-21 | **High — blocks dev-0 user migration; `d7_user_role`'s `entity:user_role` destination strips committed permissions from content_editor/authenticated/anonymous. Do NOT run the `mandala_users` group until fixed** |
