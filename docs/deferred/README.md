...
| [adr-015-unanswered-questions-at-merge.md](adr-015-unanswered-questions-at-merge.md) | access / users / migration / ADR 015 | PR #75 merge 2026-08-06 | **High (Q1)** — implementation is verified and correct, but three questions merged unanswered. Q1: **rid 6 (`shanti editor`) count is ZERO** in the scrubbed shared DB, so `content_editor` may currently apply to **nobody** while 142 real editors migrate as plain authenticated — ADR 015's own "confirm the shanti_editor population on dev-0" guardrail is unmet. Q2 (Medium): `create` is group-scoped only (bare `/node/add` denied) — a precedent every later per-site migration inherits. Q3 (Medium): `/admin` + `/admin/config` land pages reachable via retained `access administration pages` (empty shell, no privilege leak) |
