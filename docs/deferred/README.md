...
| [dev-0-code-config-delivery-rebuild-or-pipeline.md](dev-0-code-config-delivery-rebuild-or-pipeline.md) | deployment / CI-CD / dev-0 / ECR | Session 2026-07-21 | **High — no repeatable way to deliver code/config to dev-0 (image-baked, no pipeline); merging to main does not deploy. Decision manual-rebuild vs CI pipeline left OPEN for Yuji/Dave; blocks the user-migration deploy** |
...
| [adr-015-unanswered-questions-at-merge.md](adr-015-unanswered-questions-at-merge.md) | access / users / migration / ADR 015 | PR #75 merge 2026-08-06 | **Q1 RESOLVED** — 1(a) confirmed rid 6 (`shanti editor`) = 0 users on live dev-0 data + 0 code refs (2026-08-06); 1(b) decided (2026-08-07): `content_editor` migrates empty + hand-assigned, 142 rid-4 editors → plain authenticated. Confirms ADR 015 (no superseding ADR / no code change). Its High follow-through moved to the contributor-tier prerequisite. **Q2/Q3 still open (Medium):** Q2 — `create` is group-scoped only (bare `/node/add` denied), a precedent every later per-site migration inherits; Q3 — `/admin` + `/admin/config` land pages reachable via retained `access administration pages` (empty shell, no privilege leak) |
| [authenticated-contributor-crud-not-wired-in-d11.md](authenticated-contributor-crud-not-wired-in-d11.md) | access / users / migration / content model | ADR 015 Q1 follow-up 2026-08-07 | **High** — D7 authenticated users hold core (site-wide) `create` + `edit own` + `delete own` on all asset **and** collection content types (verified in Images per-site dump: rid 2 grants CRUD-own for `collection`/`subcollection`/`shanti_image`/`asset_link`/`image_agent`/`image_descriptions`/`external_classification`). D11's `authenticated` role grants **none** of it — view-only. This contributor tier is the baseline for all ~1,538 users and is what makes ADR 015's "142 editors → authenticated" non-destructive; without it, migrated users can author nothing. Collections are Groups in D11 (Group perms, not node perms) |
