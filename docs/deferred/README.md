...
| [mandala-wp-proxy-json-proxy-open-ssrf.md](mandala-wp-proxy-json-proxy-open-ssrf.md) | mandala-wp-proxy / security / Spike 6 | Spike 6 URL-strategy decision 2026-08-12 | **High — blocks generalized rollout of Option A.** `json_proxy`'s `url` param has no host allowlist (open proxy / SSRF); low-risk while Sources-only, becomes the primary high-traffic path once generalized to all sites. Fix in the external `shanti-uva/mandala-wp-proxy` repo (tagged `v1.0.0` pre-fix) before client-side generalization ships |
| [wp-kmaps-mandala-proxy-dependency.md](wp-kmaps-mandala-proxy-dependency.md) | wp-kmaps / mandala-wp-proxy / Spike 6 | Spike 6 URL-strategy decision 2026-08-12 | Medium — the two plugins stay separate (proxy isn't Mandala-specific; becoming security-sensitive), but `wp-kmaps` needs a `Requires Plugins` header + README note so the dependency isn't silently missable, as it was this session |
