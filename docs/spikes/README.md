...
| [Spike 7](spike-07-kaltura-av-integration.md) | Kaltura AV integration on Drupal 11 | Yuji | Individual | ◐ **Partial (started 2026-09-04)** — the D7-only `kaltura` module confirmed a dead end (7.x-3.2, no D9/10/11 release, Unsupported); `drupal/kaltura_media` (1.0.4) verified installable on live Drupal 11.4.5 with a one-line `core_version_requirement` patch, and its embed output confirmed structurally identical to Spike 6's live production Kaltura evidence using the real partner/uiconf/entry IDs. 3 minor non-blocking issues found (`upgrade_status`-verified). Upload/ingest path and a real migration source plugin remain open |
