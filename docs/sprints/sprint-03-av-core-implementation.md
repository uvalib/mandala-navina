...
corpus migrates and is verified locally (PR #193, ready for review). **AV6 and AV9 are
the next gate** — KMaps *display* wiring and UI, both concretely scoped below from a live
inspection of dev-0, not just "reuse the Images pattern" in the abstract. AV4's live dev-0
run finished 2026-09-10 (5h04m49s, 24/24 checks verified); AV8's live
...
2026-09-14** (see its row) — AV content is now visible in Drupal at all, which is what
surfaced that AV6/AV9's remaining work is display-layer, not data-layer: the data is
fully migrated and indexed, it just has nothing telling Drupal how to render it yet.
...
| AV6 | KMaps field *display* wiring (reuse Images pattern, already proven) | AV4 | ○ **Scoped 2026-09-14, not yet built.** All 6 KMaps fields (`field_kmap_terms`, `field_language_kmap`, `field_location`, `field_recording_location_new`, `field_subject`, `field_subcollection_new`) already exist on both bundles with data fully migrated by AV4 (confirmed: the `path`/`id`/`header`/`domain`/`defids` sub_process copy is identical to Images') — this is display config only. No `core.entity_form_display`/`core.entity_view_display` config exists at all for `audio`/`video` (Drupal falls back to auto-generated displays), so the edit widget already works (the field type's own registered default, `kmap_tree_picker`, needs no override) but viewing falls back to the plain `kmap_default_formatter` instead of the rich `kmap_popover_formatter` Images explicitly assigns. AV6 = export form/view display config for both bundles assigning `kmap_popover_formatter` to all 6 fields, mirroring `shanti_image`'s exported `core.entity_view_display.node.shanti_image.default.yml` exactly |
