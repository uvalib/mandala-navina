...
| AV9 | UI: Kaltura player field formatter; collection-content gallery variant of `shanti-thumbnail` (generalizing Sprint 2 B5) | AV1, AV4, Sprint 2 B5 (done) | ○ **Scoped 2026-09-14.** Same root cause as AV6 — no view display config exists for `audio`/`video` at all, so a migrated node currently renders via Drupal's raw default display: every field dumped in definition order, no Kaltura player embed, no layout, no gallery/thumbnail treatment. AV9 needs to build the actual `field_kaltura_player`-equivalent formatter (per AV10's configuration-layer design note above) and the view display config that uses it, plus the gallery variant of `shanti-thumbnail` for collection listings |
| AV10 | **Kaltura configuration layer** — config-driven, extensible, covering the full element set inventoried below (players/`uiconf_id`, uploader widget ui_conf, delivery, player + thumbnail dimensions, rotate/stretch), selectable **per view mode** via formatter settings. Must carry the known values and accept new ones as config, no code change | AV1 | ○ **Scoped 2026-09-14** — see the [AV10 scope note](../planning/av10-kaltura-configuration-layer.md). Confirmed by direct inspection: `drupal/kaltura_media` (contrib) already provides a working but minimal baseline (a `kaltura` field type + a formatter that embeds via `kWidget.embed()` with only entryId/partnerId/uiConfId/domain, no dimensions/delivery/rotate/stretch) — AV10 is a real greenfield build on top of that, not a rebuild. Proposed: a new `kaltura_player_config` config entity (modeled on `image_style` — named, reusable, addable without code), site-level constants in a separate `mandala_kaltura.settings` config object, secrets via the established ccrypt `container_0.env.secret` pattern (scoped in the note, actually wiring it is AV11's job) |
