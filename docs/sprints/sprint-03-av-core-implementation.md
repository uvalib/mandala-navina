...
**Status:** ◐ **In progress — started 2026-09-08.** AV2, AV3, AV4, AV5, AV6, AV8 and AV14
done (see the [AV2 scope note](../planning/av-content-type-decision.md), the
...
corpus migrates and is verified locally (PR #193, ready for review). **Two tracks are
deferred as of 2026-09-15 until Than is back (week of 2026-09-22)**: the upload track
(AV11 → AV12) and AV7's remaining two access realms — the latter because Than
understands the D7 OG/Group access requirements best, not a technical blocker either.
**What can proceed solo in the meantime**: AV9's gallery UI, AV15's metadata modal, and
AV13 (player preset data population) — all independently scoped, none touching Group
access or uploads. AV4's live
dev-0 run finished 2026-09-10 (5h04m49s, 24/24 checks verified); AV8's live
...
**AV6 closed the same day, AV9's player formatter closed 2026-09-15** — KMaps fields
now render with the same popover UX Images has, and a migrated node plays back
correctly. AV9's gallery UI variant and AV15's technical-metadata modal are the display
gaps still open.
...
| AV9 | UI: Kaltura player field formatter; collection-content gallery variant of `shanti-thumbnail` (generalizing Sprint 2 B5) | AV1, AV4, Sprint 2 B5 (done) | ◐ **Player formatter built and verified 2026-09-15** (`mandala_kaltura`'s `KalturaConfiguredFormatter` + `mandala_kaltura_player` theme hook). Deliberately faithful to D7's *actual* embed, not a richer reimagining — confirmed by reading `field_kaltura_build_embed()` in the real D7 codebase: `kWidget.embed()` only ever gets `targetId`/`wid`/`uiconf_id`/`entry_id`; `player_height`/`width` size the outer responsive container only. Verified live in DDEV against a real video node (nid 119715, real entry_id `1_jmvntv5j`): resolves the selected AV10 preset, renders the exact same embed shape D7 produced. ~~Still open: assigning this formatter in the view displays~~ **done same day** (PR #209/#210 — `field_video`/`field_audio` are wired live, verified on dev-0). **Remaining: the gallery variant of `shanti-thumbnail`.** Its one real open question — is resolved, not just estimated: `video` has no `field_thumbnail_image` at all (only `audio` does), but checking both D7's real thumbnail mechanism (`_kaltura_thumbnail_base_url()`: a predictable per-entry URL, `{server_url}/p/{partner_id}/sp/{subp_id}/thumbnail/entry_id/{entry_id}`, built from the exact same constants already in AV10's `mandala_kaltura.settings`) and `shanti-thumbnail`'s own template (`shanti-thumbnail.html.twig`) confirms it already accepts a **plain URL string** via `default_image_url` — a bare `<img src="...">` fallback, no image style, no local file, no Media entity required. So the gallery thumbnail for both bundles is just: build that URL from `entry_id` + AV10's resolver, pass it as `default_image_url`. No new field, no new migration, no new component capability — an estimate downgrade from "3–6h, medium confidence" to low-risk |
...
| AV11 | **Kaltura Session (KS) minting service** — server-side, using the official `kaltura/api-client-library` PHP SDK; short-TTL, upload-scoped KS handed to the browser. Secrets delivered at deploy time via the established ccrypt/`container_0.env.secret` pattern (see below), **never** in `config/sync` | AV1 | ⏸ **Deferred 2026-09-15 (Yuji) — paused until Than is back (week of 2026-09-22).** Not blocked technically (AV10's config layer + the secret-delivery pattern are both already scoped/proven); a capacity/sequencing call, matching the same pattern already used for Texts/Sources while Than was away. Picks up as its own track — doesn't block AV7's realms or AV15, which can proceed independently |
...
| AV15 | **PBCore/workflow technical-metadata display** — `field_workflow` + the full PBCore paragraph set (`field_pbcore_{contributor,coverage,creator,description,extension,identifier,instantiation,publisher,relation,rights_summary,sponsor,title}`) + `field_transcript`, all currently `hidden` in AV6's view display since none of them ever had a display default (created via AV3's migration, not Field UI) and neither AV6 (KMaps only) nor AV9 (player + gallery) claims them | AV3 (paragraph model), AV6, AV9 | ○ **Scoped 2026-09-15.** All are `entity_reference_revisions` (paragraph references) except `field_pbcore_rights_summary` (`text_long`) and `field_transcript` (plain `file`) — displaying them means giving each of AV3's paragraph types its own view-mode formatting too, not just assigning a formatter to the parent field. **Don't invent a new UI pattern for this — Images already proved one**: Sprint 2 B2's technical-metadata modal (`shanti_images_carousel`'s `node--shanti-image.html.twig`, a Bootstrap 5 modal populated via `hook_preprocess_node()`, 12 fields) is the direct precedent for exactly this shape (secondary/technical fields kept out of the main content flow, shown on demand). Mirror it: an AV equivalent modal (or the same one, generalized) listing these fields' paragraph data, gated the same way if any of them turn out to carry a `field_private_note`-style access restriction (check AV's actual PBCore field permissions before assuming none do — B2 found exactly one such case in Images by checking the real production DB, not assumed) |
| AV13 | Migrate D7's **per-view-mode player configuration** (`entry_widget` in each `field_config_instance` display) into AV10's registry + formatter settings — not a single site-wide default | AV10, AV4 | ○ **Not deferred with AV11/AV12** (2026-09-15) — depends on AV10/AV4 only, not AV11, and benefits AV9's formatter (more real presets to choose from) independent of uploads. Fair game to pick up now if capacity allows, though AV12 will still want its `custom_cw` values once that track resumes |
