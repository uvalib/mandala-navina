...
**Everything else solo-workable in Sprint 3 is now done** — AV4's live
...
galleries render real cards with Kaltura-derived thumbnails. AV15's technical-metadata
modal is the one display gap still open.
...
| AV15 | **PBCore/workflow technical-metadata display** — `field_workflow` + the full PBCore paragraph set (`field_pbcore_{contributor,coverage,creator,description,extension,identifier,instantiation,publisher,relation,rights_summary,sponsor,title}`) + `field_transcript`, all currently `hidden` in AV6's view display since none of them ever had a display default (created via AV3's migration, not Field UI) and neither AV6 (KMaps only) nor AV9 (player + gallery) claims them | AV3 (paragraph model), AV6, AV9 | ✅ **Done 2026-09-17.** New module `mandala_av_metadata`: an extra field (pseudo-field, not a full node template override — AV9 already owns the rest of the default view mode via display config, and nothing else here needed a layout rewrite the way Images' sibling carousel did) rendering a Bootstrap 5 trigger-link + modal, mirroring Sprint 2 B2's `shanti_images_carousel` pattern exactly. Renders paragraph fields **generically** — introspects each paragraph's own field definitions at runtime rather than hand-listing the ~85 sub-fields across AV3's 14 relevant paragraph types, so it can't drift from that config; recurses for the two nested cases (`av_pbcore_instantiation` → `av_pbcore_format_id`, `av_workflow` → three separate `av_workflow_note` streams) rather than special-casing them. Confirmed `list_string` allowed-value keys equal their labels across every field checked (spot-checked `field_title_type`'s 73 values and others against the real `d7_av` DB), so raw `.value` access is safe — no per-paragraph-type view display config needed after all, simpler than the row's original premise. **Checked AV's actual field permissions before assuming none applied, as flagged**: unlike Images' `field_private_note` (D7 `field_permissions` type 1/PRIVATE, gated on ownership), `field_workflow` is D7 `field_permissions` type 2/CUSTOM — confirmed against the real `role_permission` table that anonymous and general-authenticated roles have **no** view grant at all, only site admin/content admin/workflow editor roles do (matches the field's actual content: internal cataloguing QA flags and notes, e.g. "Needs title slates in Tibetan and Chinese"). D11 has no equivalent field-level permission wired yet, so gated on `$node->access('update')` as the stand-in, same reasoning as Images' gate. Verified live in DDEV against real nodes with populated data (115529 audio, 119715 video): PBCore sections render correctly including multi-instance paragraphs (e.g. two titles, English + Tibetan) and nested format-id/workflow-note data; `field_workflow` section confirmed present for an admin account and absent for anonymous on the same node; no new watchdog errors; `/av` and `/images` unaffected |
