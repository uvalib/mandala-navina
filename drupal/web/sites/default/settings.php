...
// Configure the default database connection from environment variables when
// provided (infrastructure/container env). Locally, DDEV's settings.ddev.php
// (included below) overrides this with the DDEV database. This keeps creds out
// of version control while letting Drush/Drupal connect during deploy.
// Expected: MYSQL_HOST, MYSQL_DATABASE, MYSQL_USER, MYSQL_PASSWORD.
 ...
 * @endcode
 */
// Set hash_salt from the environment; fallback is for local/dev only.
$settings['hash_salt'] = getenv('DRUPAL_HASH_SALT') ?: 'temporary-local-dev-only';
...
// DevOps and deployment settings (follows the drupal-dsf convention, adapted for
// the mandala-navina monorepo: the app lives under drupal/, so the committed CMI
// config dir is drupal/config/sync — i.e. '../config/sync' relative to the webroot
// (drupal/web). This resolves identically in local DDEV and in the deployed image.
// Set before the DDEV include below: DDEV's settings.ddev.php only sets
// config_sync_directory when empty, so this value wins in every environment.
